    ...
    if( (exceptionInfo != NULL) && (exceptionInfo->ExceptionRecord->ExceptionCode == EXCEPTION_BREAKPOINT) )
    {
        ...
        for( ; iter != originalMemory.end(); ++iter )
        {
            if( exceptionAddr == iter->first )
            {
                ...
                {
                    exceptionInfo->ContextRecord->Eip = iter->second.hookAddr;
                    return EXCEPTION_CONTINUE_EXECUTION;
                }
                else if( iter->second.type == SoftwareBreakpoint::TYPE_LOG ) // if type is a log
                {
                    ...
                    //wxMessageBox( wxString::Format("%08X", exceptionInfo->ContextRecord->Eip) );
    ...
    if( GetThreadContext( hThread, &context ) )
    {
        ...
        if( retVal == true ) //If we haven't failed yet
        {
            ...
            if( GetThreadIdXP( threadHandle ) != GetThreadIdXP( hThread ) )
            {
                ...
                {
//                     wxMessageBox( 
//                         wxT("Unable to SuspendThread.  Error: ") + 
//                         wxString::Format("%i", GetLastError()) );
                    retVal = false;
                }
                else if( !SetThreadContext( hThread, &context ) )
                {
//                     wxMessageBox( 
//                         wxT("Unable to SetThreadContext with CONTEXT_DEBUG_REGISTERS.  Error: ") + 
//                         wxString::Format("%i", GetLastError()) );
                    retVal = false;
                }
            }
            ...
            {
//                 wxMessageBox( 
//                     wxT("Unable to ResumeThread.  Error: ") + 
//                     wxString::Format("%i", GetLastError()) );
                retVal = false;;
            }
        }
    ...
    {
//         wxMessageBox( 
//             wxT("Unable to GetThreadContext with CONTEXT_DEBUG_REGISTERS.  Error: ") + 
//             wxString::Format("%i", GetLastError()) );
        return false;
    }
    return retVal;
                                        ...
                                        const bool hookIsTerminator)
{
    ...
    {
        wxMessageBox( 
        wxT("Failed on call to CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, 0).  Error: ") + 
        wxString::Format("%i", GetLastError()) );
        return 0;
    }
        ...
        do //For all threads
        {
            if( te32.th32OwnerProcessID == processId ) //If thread is part of process
            {
                ...
                if( hThread != NULL )
                {
                    ...
                    wxMessageBox( 
                        wxT("Unable to open thread.  Error: ") + 
                        wxString::Format("%i", GetLastError()) );
            }
...
unsigned long GetThreadIdXP(HANDLE hThread)
{
    ...
    if (hLib != 0L)
    {
        ...
        if (hProc != 0L)
        {
            ...
            {
                wxMessageBox( wxT("Failed on call to NtQueryInformationThread(hThread, 0, &tbi, sizeof(THREAD_BASIC_INFORMATION).  Error: ") +
                    wxString::Format("%08X", ret) );
            }
        }
        else 
        {
            wxMessageBox( wxT("Couldn't find to NtQueryInformationThread.  Error: ") +
                wxString::Format("%i", GetLastError()) );
        }
    ...
    {
        wxMessageBox( wxT("Failed on call to LoadLibrary(L\"ntdll.dll\").  Error: ") +
            wxString::Format("%i", GetLastError()) );
    }
