    ...
    for(int i=0; i < (int)strlen(strPacketBuf.c_str()); ++i)
            ...
            (strPacketBuf[i] == '*' ) )
        {
            ...
            if(strPacketBuf[i] == '#')
            {
                ...
                if(length > 3)
                    wxMessageBox(_("Error processing decimal representation.\nMake sure that you include a space after the decimal number\nand that the value is not over 255."), _("Error Processing Decimal"));
            }*/
...
ErrorMessage SendPacketToProcess(std::string strPacketBuf, CommunicationLayer &communicationLayer, sendPacketType pktTyp, SOCKET s)
{
    ...
    else
    {
        ...
        {
        case __SEND_PACKET:
            communicationLayer.PacketSend(cstrPacketBuffer, s); 
            break;
        case __RECV_PACKET:
            communicationLayer.PacketRecv(cstrPacketBuffer, s); 
            break;
        }
    }
