...
namespace
{
    class PacketItemWSASend : public PacketItem
    {
    public:
        PacketItemWSASend( RiPE *classPtr,
    ...
    DWORD SendFunction     = (DWORD)GetProcAddress( LoadLibraryW(L"WS2_32.dll" ), "WSASend" );
    DWORD SendFunction_Ret = SendFunction + 0x05;
    DWORD RecvFunction     = (DWORD)GetProcAddress( LoadLibraryW(L"WS2_32.dll" ), "WSARecv" );
...
SkipSendPacket:
                ret  0x1C                             //Returns, effectively blocking the packet
            }
        }
    }
    ...
    bool __stdcall SendPacketEvent( SendStructure *esp )
    {
        ...
        for( unsigned int i=0; i < esp->dwBufferCount; ++i )
        {
            ...
            PacketItemWSASend *pktItem= new PacketItemWSASend( 
                RiPEApp::GetRiPE(),
                generalPacket,
                PACKET_STREAM_SEND,
                esp->returnAddress );
...
namespace packets
{
    class PacketInterfaceWSASend : public PacketInterface
    {
    private:
    ...
    public:
        PacketInterfaceWSASend() : PacketInterface(){}

        PacketInterfaceWSASend(
            __in const PacketInterfaceWSASend &other ) :
        ...
        PacketInterfaceWSASend& operator =(PacketInterfaceWSASend &other)
        {
...
bool WSASendHookFacade::HookRecv( bool enable )
{
    bool result = true;
    return result;
}
...
// true for enable, false for disable.
// returns true if succeeded, else false
bool WSASendHookFacade::HookSend( bool enable )
{
    bool result = true;
...
void WSASendHookFacade::PacketRecv( const char *cstrPacket, SOCKET s/*=0 */ )
{
    packets::Packet<packets::PacketInterfaceWSASend> pkt(cstrPacket, packets::PACKET_STREAM_RECV);
    pkt.SendPacket(s);
}

void WSASendHookFacade::PacketSend( const char *cstrPacket, SOCKET s/*=0 */ )
{
    packets::Packet<packets::PacketInterfaceWSASend> pkt(cstrPacket);
    pkt.SendPacket(s);
}
