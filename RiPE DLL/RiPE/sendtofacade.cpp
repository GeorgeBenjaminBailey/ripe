...
namespace
{
    class PacketItemSendTo : public PacketItem
    {
    public:
        PacketItemSendTo( 
    ...
    DWORD SendFunction     = (DWORD)GetProcAddress( LoadLibraryW(L"WS2_32.dll" ), "sendto" );
    DWORD SendFunction_Ret = SendFunction + 0x05;
    DWORD RecvFunction     = (DWORD)GetProcAddress( LoadLibraryW(L"WS2_32.dll" ), "recvfrom" );

    int    flags = 0;

    int __stdcall PacketRecvHookCave( 
        SOCKET s, 
        char *buf, 
        int len, 
        int flags, 
        ...
        int *fromlen )
    {
        HookHop::FunctionHop(recvfrom, s, buf, len, flags, from, fromlen);
        __asm
        {
...
SkipSendPacket:
            mov  eax,[esp+0xC]                    // set return to the packet size
            ret  0x18                             // returns, effectively blocking the packet
        }
    }
    ...
    bool __stdcall RecvPacketEvent( RecvStructure *esp )
    {
        ...
        PacketItemSendTo *pktItem= new PacketItemSendTo( 
            generalPacket,
            PACKET_STREAM_RECV,
            esp->returnAddress );
...
namespace packets
{
    class PacketInterfaceSendTo : public PacketInterface
    {
    private:
    ...
    public:
        PacketInterfaceSendTo() : PacketInterface(){}

        PacketInterfaceSendTo(
            __in const PacketInterfaceSendTo &other ) :
        ...
        PacketInterfaceSendTo& operator =(PacketInterfaceSendTo &other)
        {
...
bool SendToHookFacade::HookRecv( bool enable )
{
    bool result = true;
    ...
    if( enable )
    {
        ...
        {
            RiPE::Container::GetBreakpoint()->SetHardwareBreakpoint( RecvFunction, (int)PacketRecvHookCave, BREAK_ON_BYTE, BREAK_ON_EXECUTION, 2 );
        }
        else
        {
    ...
    else // disable
    {
        if( usingHardwareBreakpoints )
        {
            RiPE::Container::GetBreakpoint()->ClearHardwareBreakpoint(2);
        }
        else
        {
...
// true for enable, false for disable.
// returns true if succeeded, else false
bool SendToHookFacade::HookSend( bool enable )
{
    bool result = true;
...
void SendToHookFacade::PacketRecv( const char *cstrPacket, SOCKET s/*=0 */ )
{
    packets::Packet<packets::PacketInterfaceSendTo> pkt(cstrPacket, packets::PACKET_STREAM_RECV);
    pkt.SendPacket(s);
}

void SendToHookFacade::PacketSend( const char *cstrPacket, SOCKET s/*=0 */ )
{
    packets::Packet<packets::PacketInterfaceSendTo> pkt(cstrPacket);
    pkt.SendPacket(s);
}
